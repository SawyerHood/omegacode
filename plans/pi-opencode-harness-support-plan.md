...
- **Full-access only; everything else fails closed.** Neither CLI provides OS-level sandboxing, so both providers support exactly one sandbox mode: `danger-full-access`. `read-only` and `workspace-write` are rejected for both, as is `maxTurns` (both), with `AgentError code: "unsupported_option"`, following the Codex `maxTurns` precedent (`src/worker/codex.ts:128-139`). `effort` maps to opencode `--variant` and pi `--thinking`. **Consequence:** since omegacode's default sandbox is `read-only` (`src/dsl/types.ts:129-137`), every opencode/pi call requires an explicit `sandbox: "danger-full-access"` (per call or via `--sandbox`) — the rejection error says so, and docs flag it prominently.
...
| `effort` set | Forward as `--variant <effort>`. `--variant` is documented as provider-specific reasoning effort (`run.ts:208-211`); unsupported values remain the provider's responsibility, matching other open model/provider options. | argv includes `--variant`, `<effort>` |
...
| OpencodeWorker | `test/opencode-worker.test.ts` | argv construction (model with nested slashes; flag omission when model unset; `--thinking` always present; `effort` forwarded as `--variant`; `--dangerously-skip-permissions` only for danger); stdin prompt incl. instructions-preamble injection; full event-mapping table incl. reasoning and tool completed/error pairs; **`error` event fatal on exit 0**; exit 0 + no text → `no_result`; usage accumulation across `step_finish`; rejections (`read-only` with actionable message, `workspace-write`, `maxTurns`); version preflight (stubbed `--version` below 1.16.2 → `provider_outdated`, checked once per worker); schema extraction hit/miss + `--session` reuse + instructions forwarded to extraction + usage summing; corrective-retry instructions reach the prompt preamble; abort/stall |
...
7. *(Optional follow-up, out of v1 scope)* `omegacode models` dynamic discovery (`opencode models --verbose`, pi RPC `get_available_models`), deeper doctor auth probes, persistent-mode workers.
...
- Fail-closed semantics enforced and tested: both providers reject `read-only` and `workspace-write` (full-access only) and `maxTurns`; opencode forwards `effort` as `--variant`; opencode permission asks are auto-rejected except under `danger-full-access`.
...
1. ~~**opencode `effort → --variant` mapping.**~~ **Resolved:** forward `effort` as `--variant <effort>`. `--variant` is verified as "model variant (provider-specific reasoning effort, e.g., high, max, minimal)" flowing into the session prompt input (`run.ts:208-211, 487-496, 782-856`). Unsupported values remain provider/model behavior, matching other open provider-specific options. (`--thinking` is resolved: boolean display gate, always passed.)
